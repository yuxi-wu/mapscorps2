...
def get_scores(url):
    fullurl = 'https://www.walkscore.com' + url
    soup = get_strained_soup(fullurl,'td')
    scores = soup.find('a',href=url)
    if scores:
        ...
        if 'Chicago' in url:
            fullurl = 'https://www.walkscore.com/score/' + url.strip('/IL/Chicago/') \
                + '-Chicago'

            ws = get_strained_soup(fullurl, 'img').find(alt=
                'Walk Score of this location')
            walkscore = 0
            if ws:
                walkscore = int(ws['src'].strip('//pp.walk.sc/badge/walk/score/').strip('.svg'))

            ts = get_strained_soup(fullurl, 'img').find(alt=
                'Transit Score of this location')
            transitscore = 0
            if ts:
                transitscore = int(ts['src'].strip('//pp.walk.sc/badge/transit/score/').strip('.svg'))

            bs = get_strained_soup(fullurl, 'img').find(alt=
                'Bike Score of this location')
            bikescore = 0
            if bs:
